    ...
    <div class="d-flex align-items-center">
        ...
        <div class="flex-grow-1">
            ...
            <small class="text-muted d-block">
               Vence: @formatoFecha($cliente->fecha_firma)
            </small>
        </div>
    </div>
</a>
...
<div class="modal fade" id="modalNotificacion{{ $cliente->id_clientes }}" tabindex="-1" aria-hidden="true" data-bs-backdrop="false">
            ...
            <div class="modal-body">
                <form action="{{ route('notificar.cliente') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <p><strong>Fecha de vencimiento:</strong> @formatoFecha($cliente->fecha_firma) (en {{ round($diasRestantes) }} días)</p>
                    </div>
                    
                    <div class="mb-3">
                        <label for="mensaje{{ $cliente->id_clientes }}" class="form-label">Mensaje:</label>
                        <textarea class="form-control" name="mensaje" id="mensaje{{ $cliente->id_clientes }}" rows="5" required>Estimado/a {{ $cliente->nombre_cliente }},

Le informamos que su Firma Electrónica vencerá en {{ round($diasRestantes) }} días (@formatoFecha($cliente->fecha_firma))
Por favor contáctenos para renovación.</textarea>
                    </div>
                    
                    <!-- <div class="mb-3">
                        <label for="archivo{{ $cliente->id_clientes }}" class="form-label">Adjuntar archivo (opcional):</label>
                        <input type="file" class="form-control" name="archivo" id="archivo{{ $cliente->id_clientes }}">
                    </div> -->
